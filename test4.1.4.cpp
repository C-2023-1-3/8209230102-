...
void merge(const int list1[], int size1, const int list2[], int size2, int list3[])
{
	...
	for (int i = 0; i < size3; i++)//建立循环 对list3赋值
	{
		if (i < size1)//如果i<list1的长度 将list1中的数值依次赋给list3
			list3[i] = list1[i];
		else//i超出了list1的长度 将list2中的数值依次赋给list3
